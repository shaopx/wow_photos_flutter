...
class _MyPaintWidgetState extends State<MyPaintWidget> {
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () {
      _loadImage(MediaQuery.sizeOf(context).width.toInt(),
  ...
  Future<ui.Image> _loadImage(int width, int height) async {
    final ByteData data = await rootBundle.load('assets/images/test.webp');
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (image != null) {
      ...
      if (screenRatio >= imageRatio) {
        ...
        // 需要上下裁剪图片
        int targetImageHeight = size.height.toInt();
        int heightDiff = imageHeight - targetImageHeight;
        print('targetImageHeight:$targetImageHeight, heightDiff:$heightDiff');
        canvas.drawImageRect(
            image!,
